...
bool URivenSaveGame::GetBooleanBySaveGameField(ESaveGameField fieldName) {
	...
	[[[cog
		...
		booleans = [var for var in vars if var[0] == 'bool']
		p('if (')
		for i, b in enumerate(booleans):
			last = i == len(booleans) - 1
			p('fieldName == ESaveGameField::%s%s' % (b[1], ' ||' if not last else ''))
		p(') {')
	]]] */
	if (

	fieldName == ESaveGameField::CurrentZoomedLinkingBookIsOpen ||

	fieldName == ESaveGameField::GehnIsTrapped ||

	fieldName == ESaveGameField::HaveTrapBook ||

	fieldName == ESaveGameField::CatherineIsFree ||

	fieldName == ESaveGameField::J_SeenCreepyGirl ||

	fieldName == ESaveGameField::O_OvenIsOn ||

	fieldName == ESaveGameField::O_CageIsUp ||

	fieldName == ESaveGameField::T_NewTemple_DoorMain_Open ||

	fieldName == ESaveGameField::T_NewTemple_DoorSide_Open ||
