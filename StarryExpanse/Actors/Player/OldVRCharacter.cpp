...
AOldVRCharacter::AOldVRCharacter(const FObjectInitializer &ObjectInitializer)
  ...
  m_pChaperone->OnLeaveBounds.AddDynamic(this,
                                         &AOldVRCharacter::OnLeaveVRBounds);
  m_pChaperone->OnReturnToBounds.AddDynamic(
      this, &AOldVRCharacter::OnReenterVRBounds);
}

void AOldVRCharacter::BeginPlay() {
  // spawn hands
...
void AOldVRCharacter::SetupPlayerInputComponent(
    UInputComponent *PlayerInputComponent) {
  Super::SetupPlayerInputComponent(PlayerInputComponent);
}

void AOldVRCharacter::Tick(float DeltaTime) {
...
void AOldVRCharacter::OnLeaveVRBounds() {
  if (GEngine)
    GEngine->AddOnScreenDebugMessage(
        -1, 5.f, FColor::Cyan,
        TEXT("[AOldVRCharacter] Player has left VR bounds"));
}

void AOldVRCharacter::OnReenterVRBounds() {
  if (GEngine)
    GEngine->AddOnScreenDebugMessage(
        -1, 5.f, FColor::Cyan,
        TEXT("[AOldVRCharacter] Player has reentered VR bounds"));
}
#pragma endregion VRBounds

// void AOldVRCharacter::GetLifetimeReplicatedProps(
//     TArray<FLifetimeProperty> &OutLifetimeProps) const {
//   Super::GetLifetimeReplicatedProps(OutLifetimeProps);

//   DOREPLIFETIME(AOldVRCharacter, m_pLeftHand);
//   DOREPLIFETIME(AOldVRCharacter, m_pRightHand);
//   DOREPLIFETIME(AOldVRCharacter, m_pCapsuleTrigger);
// }

void AOldVRCharacter::ToggleVR() {
  TSharedPtr<IXRTrackingSystem, ESPMode::ThreadSafe> pXRSystem{
      GEngine->XRSystem};
  if (pXRSystem) {
    IHeadMountedDisplay *pHMD{GEngine->XRSystem->GetHMDDevice()};
...
void AOldVRCharacter::ToggleStereo() {
  TSharedPtr<IXRTrackingSystem, ESPMode::ThreadSafe> pXRSystem{
      GEngine->XRSystem};
  if (pXRSystem) {
    TSharedPtr<IStereoRendering, ESPMode::ThreadSafe> pStereo{
...
void AOldVRCharacter::InitializeNewMotionController(
    AVRHand *pVRHand, EControllerHand trackedHand) {
  if (!pVRHand) {
    if (GEngine)
      GEngine->AddOnScreenDebugMessage(
          -1, 15.f, FColor::Red,
          TEXT(
              ...
              "AOldVRCharacter::InitializeNewMotionController()"));
    return;
  }
