...
void IRivenInteractable::Drag_Update_Implementation(FVector2D Origin,
                                                    FVector2D SmallDelta,
                                                    FVector2D OverallDelta) {}

FDragCallbackPreferences IRivenInteractable::Drag_Begin_Implementation(
    FHitResult HitInfo, FVector2D Origin, AActor *DragOwner) {
  return FDragCallbackPreferences::ModerateDelay_Simple();
}

void IRivenInteractable::Extra_Drag_Details_Available_Implementation(
    FHitResult HitInfo) {}

void IRivenInteractable::Drag_Finished_Implementation(FHitResult HitInfo,
                                                      FVector2D Origin,
                                                      FVector2D SmallDelta,
                                                      FVector2D OverallDelta,
                                                      bool WasDragCanceled) {}

void IRivenInteractable::Touched_Implementation() {}

FInteractabilityProbeResponse
IRivenInteractable::ProbeInteractability_Implementation() {
